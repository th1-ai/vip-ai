...
You are working inside **VIP AI** ("The Insider") — One system that makes every VIP feel known..
...
**Main workflow.** `workflows/10-vip-brief.md` — `python3 tools/run.py --once`
syncs VIP profiles from the PMS and builds a brief for every arrival inside
`brief.lookahead_days`. There is no LLM call anywhere in this loop; it is
deliberately deterministic (`docs/how-it-works.md`).

**The one LLM call in this repo.** `python3 tools/research.py add` — a
human pastes a research note they already found (this repo does not scrape
anything), and the model tidies it into a headline/body and flags anything
uncertain or guardrail-adjacent. It never runs on a schedule.

**Sub-agent.** Handwritten Letter AI (`tools/letters.py`,
`workflows/20-handwritten-letter.md`) is off by default —
`subagents.handwritten_letter.enabled` in `config/agent.yaml`. Ask before
turning it on; a physical letter is a real cost, not a free extra
(`docs/sub-agents.md`).

**What needs a human.** Every VIP brief and every letter waits in the review
queue (`workflows/80-review.md`) — there is no auto-send path for either
kind. A privacy-flagged profile, an unconfirmed research match, or a postal
letter with no address on file all route to `needs_human` specifically, not
just `pending_review` — treat those as "read this one first."

**No robot writer, no scraper.** If asked to "connect the handwriting robot"
or "make it search LinkedIn automatically," say plainly that this build
does not do either and point at `docs/how-it-works.md`'s design decisions
#1 and #13 for why, and `docs/integrations.md` for how to wire in your own.
